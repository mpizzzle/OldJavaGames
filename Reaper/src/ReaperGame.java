...
public class ReaperGame extends Applet implements KeyListener, Runnable {
    ...
    int isEnemyPosition(Point p) {
        for (int i = 0 ; i < preyNumber; i++) {
            if(isEnemyX(p) != -1 && isEnemyY(p) != -1 && !preyDead[i]) return(i);
        }
        return(-1);
    }
    ...
    int isObstacleX(Point p, boolean forReaper) {
        int obstaclex; 
        for (int i = 0 ; i < obstacleNumber; i++) {
            // if x is >= obstacleposition - (REAPERLEFTOFOBSTACLE) and x <= obstacleposition + (REAPERRIGHTOFOBSTACLE)
    ...
    boolean isBlockingObstacleY(Point p, int obstaclenumber) {
        String s = "Is Reaper blocking obstacle " + obstaclenumber + " which is " + (isTall[obstaclenumber] ? "" : "not ") + "tall";
    ...
    int isEnemyX(Point p) {
        ...
        for (int i = 0 ; i < preyNumber; i++) {
            ...
            //    p.x + " >= " + (preyx - REAPERLEFTOFPREY) + " && " + p.x + " <= " + (preyx + REAPERRIGHTOFPREY));
    ...
    int isEnemyY(Point p) {
        ...
        for (int i = 0 ; i < preyNumber; i++) {
            if(!preyDead[i] &&
               ((p.y+offset <= preyPositions[i].y+preySize[preyType[i]]) &&
    ...
    public void run() {
        ...
        while (true) {
            ...
            if (ingame) {
                ...
                for (int i=0; i < preyNumber; i++) {
                    // TODO if there is a fatal obstacle at that position, fall into it and decrement count of prey
                    // and set that prey's position to -1, -1
                    /*
                    if (isObstaclePosition(preyPositions[i])) {
                        showStatus("A prey fell into a obstacle");
                        
                        preyPositions[i] = new Point(-1, -1);
                        preyRemaining--;
                        showStatus(preyNumber-preyRemaining + " down, " + preyRemaining + " to go...");
                    }
                    */
                    ...
                    if (preyno > -1) {
                        ...
                        if (easymode) NEEDTOBEABOVE = preyPositions[preyno].y-preyHeight[preyType[preyno]];
                        if ((!onTheWayUp && (playerPosition.y <=NEEDTOBEABOVE))) {
                            ...
                            if (playerPosition.y > NEEDTOBEABOVE && 
                               (playerPosition.y+offset <= preyPositions[preyno].y+preySize[preyType[preyno]])) {
                                die();
                                setSomethingChangedSinceRepaint(true);
                            }
                        }
    ...
    boolean isVisibleEnemy(int i) {
        ...
        return ((preyPositions[i].x >= page*PAGEWIDTH) && 
            (preyPositions[i].x < (page+1)*PAGEWIDTH) && !preyDead[i]);
    }
    ...
    synchronized void incrementHeroImage() {
        reaperImageNo++;
        if (reaperImageNo == HEROIMAGES) reaperImageNo = 0;
    }
    
    synchronized void incrementEnemyImage(int i) {
        preyImageNo[i]++;
        if (preyImageNo[i] > (PREYIMAGES-2) ) preyImageNo[i] = 0;
    }
    
    Point move(int preyNo, Point start, int direction, boolean odd) {
        if (odd) incrementEnemyImage(preyNo);
        if (direction == LEFT) {
    ...
    void displayStatus() {
        /*
        // when a certain F Key is pressed, display positions of Reaper, 
        // all obstacles and all prey and the end flag (warp zone)
        System.out.println("--------");
        System.out.println("Reaper is " + (dead? "dead" : "alive"));
        if (dead) {
            int preyno = isEnemyPosition(playerPosition);
            System.out.println("Reaper was killed by prey " + preyno);
        }
        System.out.println("In game is " + ingame);
        System.out.println("Reaper is " + (onTheWayUp? "on the way up" : "not on the way up"));
        System.out.println("Reaper's position is (" + playerPosition.x + ", " + playerPosition.y + ")");
        System.out.println("Reaper's visible left, bottom co-ordinates are " + (playerPosition.x + 55) + " and " + (playerPosition.y + REAPERHEIGHT));
        System.out.println("Reaper is on page " + page + " (which shows x positions from " + (page*PAGEWIDTH) + " to " + ((page+1)*PAGEWIDTH) + ")");
        System.out.println("Screen bounds are (" + getBounds().width + ", " + getBounds().height + ")");
        System.out.println("");
        for (int prey=0; prey < preyNumber; prey++) {
            System.out.println("Enemy " + prey + " is " + preyTypeString(preyType[prey]) + " and is " + 
                (preyDead[prey] ? " " : "not ") + "dead");
            System.out.println("Enemy " + prey + " is at position (" + preyPositions[prey].x + 
    ...
    void changePageIfNecessary() {
        ...
        if ((playerPosition.x+SPACE_FROM_X_TO_REAPER) > (page*PAGEWIDTH+RIGHTEDGE-XAMOUNT)) {
            //System.out.println("Change page up: (" + playerPosition.x+ "+" + SPACE_FROM_X_TO_REAPER+ ") > ("  + page+ "*" + PAGEWIDTH + "+" + RIGHTEDGE + "-" + XAMOUNT + ")");
            page++;
        }
        else if ((playerPosition.x+SPACE_FROM_X_TO_REAPER)-(page*PAGEWIDTH) < (LEFTEDGE+XAMOUNT)) {
            //System.out.println("Change page down: (" + playerPosition.x+ "+" + SPACE_FROM_X_TO_REAPER+ ")-(" + page+ "*" + PAGEWIDTH + ") < (" + LEFTEDGE+ "+" + XAMOUNT+")");
           page--;
        }
    }
}
